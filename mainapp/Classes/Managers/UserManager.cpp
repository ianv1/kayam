...
string UserManager::getCurrentLevelID() {
    if (_currentLevelID.length() == 0) {
        _currentLevelID = UserDefault::getInstance()->getStringForKey(
                getCurrentLevelIDKey().c_str(), "");
    }
...
void UserManager::setPretestProgressType(string levelID, PretestProgressType type) {
    ...
    if (cur->categoryLevel > 0 && cur->categoryLevel <= 4) {
        UserDefault::getInstance()->setIntegerForKey(
                (_userName + "pretest_progress_" + levelID).c_str(), (int) type);
    }
    return;
}

guideCoopType UserManager::getGuideCoopStatus() {
    return (guideCoopType) UserDefault::getInstance()->getIntegerForKey(
            (_userName + "guide_coop_flag").c_str(), (int) guideCoopType::visitFirst);
}

void UserManager::setGuideCoopStatus(guideCoopType type) {
    UserDefault::getInstance()->setIntegerForKey((_userName + "guide_coop_flag").c_str(),
                                                 (int) type);

}

guideDayType UserManager::getGuideDayStatus() {
    return (guideDayType) UserDefault::getInstance()->getIntegerForKey(
            (_userName + "guide_day_flag").c_str(), (int) guideDayType::touchFirst);
}

void UserManager::setGuideDayStatus(guideDayType type) {
    UserDefault::getInstance()->setIntegerForKey((_userName + "guide_day_flag").c_str(),
                                                 (int) type);
}
...
bool UserManager::isDayCleared(string levelID, int day) {
    ...
    if (it == _dayClearedMap.end()) {
        cleared = UserDefault::getInstance()->getBoolForKey(getDayClearedKey(levelID, day).c_str(),
                                                            false);
...
bool UserManager::checkIfNextDayIsAvailable(string levelID, int day) {
    ...
    for (int num = 0; num <= numOfGamesInDay; num++) {
        if (UserManager::getInstance()->isGameCleared(levelID, _currentDay,
                                                      num))
            clearedNumOfGamesInDay++;
    }
...
string UserManager::getGameClearedKey(string levelID, int day, int gameIndex) {
    return _userName + "_gameCleared_" + levelID + "_" + TodoUtil::itos(day) + "_" +
           TodoUtil::itos(gameIndex);
}
...
bool UserManager::isGameCleared(string levelID, int day, int gameIndex) {
    ...
    if (it == _gameClearedMap.end()) {
        cleared = UserDefault::getInstance()->getBoolForKey(
                getGameClearedKey(levelID, day, gameIndex).c_str(), false);
...
void UserManager::setGameCleared(string levelID, int day, int gameIndex, bool isCleared) {
    ...
    UserDefault::getInstance()->setBoolForKey(
            getGameClearedKey(levelID, day, gameIndex).c_str(), isCleared);
    UserDefault::getInstance()->flush();
}
...
void UserManager::setFishPresentCurrentProgressLevel(string levelID, int levelIndex) {
    ...
    UserDefault::getInstance()->setIntegerForKey(
            getFishPresentCurrentProgressLevelKey(levelID).c_str(), levelIndex);
    UserDefault::getInstance()->flush();
}

int UserManager::getFishPresentCurrentProgressLevel(string levelID) {
    return UserDefault::getInstance()->getIntegerForKey(
            getFishPresentCurrentProgressLevelKey(levelID).c_str());
}
...
void UserManager::setFishPresentCurrentProgressIndex(string levelID, int levelIndex, int index) {
    ...
    UserDefault::getInstance()->setIntegerForKey(
            getFishPresentCurrentProgressIndexKey(levelID, levelIndex).c_str(), index);
    UserDefault::getInstance()->flush();
}
...
int UserManager::getFishPresentCurrentProgressIndex(string levelID, int levelIndex) {
    return UserDefault::getInstance()->getIntegerForKey(
            getFishPresentCurrentProgressIndexKey(levelID, levelIndex).c_str());
}
...
bool UserManager::isFishPresentEnable(string levelID) {
    return UserDefault::getInstance()->getBoolForKey(getFishPresentEnableKey(levelID).c_str(),
                                                     true);
}
...
const std::string UserManager::getAppVersion() {
    ...
    JniMethodInfo t;
    std::string a("");
    if (JniHelper::getStaticMethodInfo(t, "org/cocos2dx/cpp/AppActivity", "getAppVersion",
                                       "()Ljava/lang/String;")) {
...
const std::string UserManager::getCurrentUsername() {
    ...
    JniMethodInfo t;
    std::string a("");
    if (JniHelper::getStaticMethodInfo(t, "org/cocos2dx/cpp/AppActivity", "getCurrentUsername",
                                       "()Ljava/lang/String;")) {
        jstring jstr = (jstring) t.env->CallStaticObjectMethod(t.classID, t.methodID);
        a = JniHelper::jstring2string(jstr);
        t.env->DeleteLocalRef(jstr);
...
void UserManager::finishTutorial() {
    ...
    JniMethodInfo t;
    if (JniHelper::getStaticMethodInfo(t, "org/cocos2dx/cpp/AppActivity", "finishTutorial",
                                       "()V")) {
...
void UserManager::sendAppToBack() {
    ...
    bool result = JniHelper::getStaticMethodInfo(t, "org/cocos2dx/cpp/AppActivity", "sendToBack",
                                                 "()V");
    if (result) {
        t.env->CallStaticVoidMethod(t.classID, t.methodID);
        t.env->DeleteLocalRef(t.classID);
