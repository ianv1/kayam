#!/usr/bin/env python3
"""
Script to translate English audio files in numbertrain/sounds to Malay audio.
This script translates specific number train instructions to Malay.
"""
...
# English to Malay translations for number train audio
...
def get_malay_text(english_text):
    """Get Malay translation for English number train instruction."""
    return NUMBERTRAIN_TRANSLATIONS.get(english_text, english_text)
...
def process_audio_files(sound_dir):
    ...
    for audio_file in sound_path.glob('*.m4a'):
        ...
        instruction_name = audio_file.stem  # filename without extension
        
        if instruction_name not in NUMBERTRAIN_TRANSLATIONS:
        ...
        malay_text = get_malay_text(instruction_name)
        print(f"  📝 English: '{instruction_name}' → Malay: '{malay_text}'")
...
def main():
    parser = argparse.ArgumentParser(description='Translate English audio files to Malay for numbertrain game')
    parser.add_argument('sound_dir', nargs='?',
                       help='Path to the sound directory containing audio files',
                       default='/Users/ian/projects/kayam/kayam/mainapp/Resources/localized/ms-my/games/numbertrain/sounds/')
    parser.add_argument('--dry-run', action='store_true',
                       help='Show what would be processed without making changes')
    
    args = parser.parse_args()
    
    print("🚂 Number Train Audio Translation Tool")
    print("=" * 50)
    ...
    if args.dry_run:
        ...
        if sound_path.exists():
            for audio_file in sound_path.glob('*.m4a'):
                filename = audio_file.name
                instruction_name = audio_file.stem
                if instruction_name in NUMBERTRAIN_TRANSLATIONS:
                    malay_text = get_malay_text(instruction_name)
                    print(f"🔄 Would process: {filename} ('{instruction_name}' → '{malay_text}')")
                else:
